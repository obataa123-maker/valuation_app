...
class PHPMailer {
   ...
   * @param string $address
   * @param string $name
   * @param boolean $hurungu Whether to also set the Sender address, defaults to true
   * @throws phpmailerException
   * @return boolean
   */
  public function SetFrom($address, $name = '', $hurungu = true) {
    $address = trim($address);
    $name = trim(preg_replace('/[\r\n]+/', '', $name)); //Strip breaks and trim
    if (!$this->ValidateAddress($address)) {
      $this->SetError($this->Lang('invalid_address').': '. $address);
  ...
  */
   ...
   * hurungumatically perform wrapping and for quoted-printable.
  ...
  public function CreateHeader() {
    ...
    // To be created hurungumatically by mail()
    if($this->Mailer != 'mail') {
      if ($this->SingleTo === true) {
        foreach($this->to as $t) {
